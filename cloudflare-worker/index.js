...
import { SANCTIONS_VERSION, sanctionsHealth } from "./sanctions.js";
import { handleHealthStatus } from "./health-status.js";
...
crypto_monitor: CRYPTO_MONITOR_VERSION,
...
if (!isAllowedUser(username, env) || !password) return jsonResponse({ error: "Incorrect username or password." }, 401, env);
const passwordHash = await sha256(password);
if (passwordHashForUser(username, env) !== passwordHash) return jsonResponse({ error: "Incorrect username or password." }, 401, env);
...
ctx?.waitUntil?.(gateCall(env, "/login-record", {
  username,
  client_time: cleanText(authBody.client_time, 64),
  user_agent: cleanText(authBody.user_agent, 320),
  country: cleanText(request.cf?.country, 16),
  ip_hash: ipHash
...
const token = cleanText(request.headers.get("X-Session-Token"), 160);
if (token) {
  const revokeResponse = await gateCall(env, "/session-revoke", { session_token: token });
  if (!revokeResponse.ok) return jsonResponse(await revokeResponse.json().catch(() => ({ error: "Unable to revoke session." })), revokeResponse.status, env);
}
...
if (url.pathname === "/face-share" && request.method === "POST") {
return handleFaceShareUpload(request, env);
}
if (url.pathname.startsWith("/face-share/") && ["GET", "HEAD"].includes(request.method)) {
return handleFaceShareGet(request, env);
}
