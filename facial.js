...
function faceTable(faces){
 ...
 return '<table class="face-table"><thead><tr><th>Face</th><th>Quality</th><th>Sharpness</th><th>Brightness</th><th>Image area</th></tr></thead><tbody>'+
 faces.map(f=>'<tr><td>F'+esc(f.face_id)+'</td><td><span class="quality">'+esc(f.quality)+' '+esc(f.quality_score)+'</span></td><td>'+esc(f.sharpness)+'</td><td>'+esc(f.brightness)+'</td><td>'+esc((Number(f.size_ratio||0)*100).toFixed(1))+'%</td></tr>').join("")+'</tbody></table>';
}
...
function render(payload){
 ...
 $("fiItems").innerHTML=files.map(f=>{
  ...
  if(f.kind==="video"){
   return '<section class="media-item"><div class="media-head"><h3>'+esc(f.filename)+'</h3><div class="media-meta">VIDEO · '+esc(f.duration_seconds)+'s · '+esc(f.fps)+' FPS · '+esc(f.face_count)+' detections</div></div><div class="preview-grid">'+(f.sampled_frames||[]).map(previewCard).join("")+'</div></section>';
  }
  return '<section class="media-item"><div class="media-head"><h3>'+esc(f.filename)+'</h3><div class="media-meta">IMAGE · '+esc(f.width)+'×'+esc(f.height)+' · '+esc(f.face_count)+' face(s)</div></div><div class="preview-grid">'+previewCard(f)+'</div>'+exif+'</section>';
 }).join("");
